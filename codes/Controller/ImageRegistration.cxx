...
bool ImageRegistration::Update()
{
	...
	if (this->CheckSameSpacing())
	{
		// Do not register if the image has the same spacing, direction and origin
		std::cout << __FUNCDNAME__ << ": Detected same spacing, skipping registration!" << std::endl;
		m_outputImage = ImageType::New();
		m_outputImage->Graft(m_movingImage);
	}
	else {
...
bool ImageRegistration::CheckSameSpacing()
{
	return (
		m_movingImage->GetOrigin() == m_fixedImage->GetOrigin() &&
		m_movingImage->GetDirection() == m_fixedImage->GetDirection() &&
		m_movingImage->GetSpacing() == m_fixedImage->GetSpacing()
		);
}
